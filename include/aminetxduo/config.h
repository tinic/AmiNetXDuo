 ...
 * next: it is current, and somebody configured the server on purpose. An
 * interface file's ID= names an interface and the machine only by implication,
 * but an administrator did write it. ENV:HOSTNAME is last -- it is ambient, it
 * survives a disk moving between machines, and a remnant one is what kept a
 * renamed machine answering to its old name.
 */
typedef enum {
