#!/usr/bin/env bash
# A socket rejoins after its physical interface is removed and re-added in
# the same NetX slot.  The old BSD row must not survive NetX's implicit leave.
# Emulator-only: McastProbe EPOCH invokes Remove/Add on zeth1 while open.
...
if [ "$BACKEND" = slirp ] || [ "$BACKEND" = slirp_inbound ]; then
    echo "mcastepoch needs a bridged a2065 interface" >&2
    exit 2
fi

TOOLS="$BUILD/src/tools"
BSD="$BUILD/src/bsdsocket/bsdsocket.library"
for file in "$TOOLS/ToolsSmoke" "$TOOLS/AddNetInterface" \
...
if [ -z "$ADDRESS" ]; then
    rig_claim_address "${AMINETXDUO_RIG_ADDR_PREFIX:-192.168.1}" \
        ...
        "${AMINETXDUO_RIG_ADDR_LAST:-254}" "run-mcastepoch in $ROOT" || exit 2
    ADDRESS="$RIG_ADDRESS"
fi

STAGE=$(mktemp -d "$ROOT/build/mcastepoch-stage.XXXXXX") || exit 2
...
printf 'DEVICE=a2065.device\nUNIT=0\nCONFIGURE=STATIC\nADDRESS=%s\nNETMASK=255.255.255.0\n' \
    "$ADDRESS" > "$STAGE/devs/NetInterfaces/zeth1"

{
    echo 'SYS:AddNetInterface zeth1'
    echo "SYS:McastProbe EPOCH $ADDRESS"
...
(
    ...
    "$ROOT/tools/amiberry-run.sh" -N a2065 -B "$BACKEND" -m A1200 \
        ...
        "$TOOLS/RemoveNetInterface" "$STAGE/McastProbe"
)
RUN_RC=$?
