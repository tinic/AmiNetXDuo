/*
 * inline/bsdsocket.h, replacement for the Roadshow NDK header: the same ABI in
 * inline/macros.h LP<n> form ("special 40"), because the NDK's "special 46"
 * form is rejected by GCC 15.  fd2pragma 2.197; do not hand-edit.
 */
...
#define connect(sock, name, namelen) \
	LP3(0x36, LONG, connect, LONG, sock, d0, struct sockaddr *, name, a0, socklen_t, namelen, d1, \
	, BSDSOCKET_BASE_NAME)

#define sendto(sock, buf, len, flags, to, tolen) \
	LP6(0x3c, LONG, sendto, LONG, sock, d0, APTR, buf, a0, LONG, len, d1, LONG, flags, d2, struct sockaddr *, to, a1, socklen_t, tolen, d3, \
	, BSDSOCKET_BASE_NAME)
...
#define setsockopt(sock, level, optname, optval, optlen) \
	LP5(0x5a, LONG, setsockopt, LONG, sock, d0, LONG, level, d1, LONG, optname, d2, APTR, optval, a0, socklen_t, optlen, d3, \
	, BSDSOCKET_BASE_NAME)
...
#define WaitSelect(nfds, read_fds, write_fds, except_fds, timeout, signals) \
	LP6(0x7e, LONG, WaitSelect, LONG, nfds, d0, APTR, read_fds, a0, APTR, write_fds, a1, APTR, except_fds, a2, struct __timeval *, timeout, a3, ULONG *, signals, d1, \
	, BSDSOCKET_BASE_NAME)
...
#define Inet_LnaOf(in) \
	LP1(0xba, in_addr_t, Inet_LnaOf, in_addr_t, in, d0, \
	, BSDSOCKET_BASE_NAME)

#define Inet_NetOf(in) \
	LP1(0xc0, in_addr_t, Inet_NetOf, in_addr_t, in, d0, \
	, BSDSOCKET_BASE_NAME)

#define Inet_MakeAddr(net, host) \
	LP2(0xc6, in_addr_t, Inet_MakeAddr, in_addr_t, net, d0, in_addr_t, host, d1, \
	, BSDSOCKET_BASE_NAME)
...
#define getnetbyaddr(net, type) \
	LP2(0xe4, struct netent *, getnetbyaddr, in_addr_t, net, d0, LONG, type, d1, \
	, BSDSOCKET_BASE_NAME)
...
#define gethostname(name, namelen) \
	LP2(0x11a, int, gethostname, char*, name, a0, size_t, namelen, d0, \
	, BSDSOCKET_BASE_NAME)

#define gethostid() \
	LP0(0x120, long, gethostid, \
	, BSDSOCKET_BASE_NAME)
...
#define In_LocalAddr(address) \
	LP1(0x264, LONG, In_LocalAddr, in_addr_t, address, d0, \
	, BSDSOCKET_BASE_NAME)

#define In_CanForward(address) \
	LP1(0x26a, LONG, In_CanForward, in_addr_t, address, d0, \
	, BSDSOCKET_BASE_NAME)
...
/*
 * Hand-added Roadshow tail: the generating FD stops at ObtainServerSocket().
 * Register assignment is Roadshow's.
 */
