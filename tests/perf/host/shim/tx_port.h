 ...
 * function under test at all.
 *
 * ThreadX's Linux port already does exactly this for x86_64 -- 32-bit
 * LONG/ULONG, 64-bit ALIGN_TYPE so a pointer still fits -- but the branch is
 ...
 * would be a lie the system headers would eventually notice, so instead the
 * port's own typedefs are renamed out of the way on the way in and replaced
 * afterwards.  Nothing here is linked; the shim exists so ONE vendored
 * translation unit compiles.
 *
 * Endianness is deliberately left as the Linux port has it (little).  The
 * m68k is big-endian, but both implementations consult the same
