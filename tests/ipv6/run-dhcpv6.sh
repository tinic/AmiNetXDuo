...
if [ -n "$chosen_server" ] && [ "$chosen_server" != "$PEER_LL" ]; then
    ...
    echo "That question has a harness of its own now, tests/ipv6/"
    echo "run-dhcpv6-pref.sh, which brings a server that can.  Every"
    echo "assertion after this point would be"
