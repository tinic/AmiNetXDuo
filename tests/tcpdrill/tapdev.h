 ...
 * number, any window, any option, in any order, at any time.
 *
 ...
 * which is the correct instrument for conformance and the wrong one for
 * throughput.  Throughput lives in tests/trace/, over the real A2065.
 *
...
/* How many transmitted frames may sit uncollected.  A capture that wraps is a
   capture that lies, so overruns are counted and reported rather than hidden;
   the script engine drains after every step, so this only has to cover a burst
   the guest emits inside one poll interval. */
 ...
 * when no CMD_READ of that EtherType was outstanding -- which is a real
 * SANA-II behaviour and not an error here, so the caller decides what it means.
 */
