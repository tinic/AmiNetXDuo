/*
 * Test certificates for the loopback TLS handshake, lifted verbatim from
 * third_party/netxduo/samples/demo_netx_secure_tls.c (MIT, Microsoft /
 * Eclipse ThreadX).
 *
 *   test_device_cert_der      leaf certificate, RSA-2048, signed by the CA below
 *   test_device_cert_key_der  its private key, PKCS#1 DER
 *   test_ca_cert_der          the self-signed CA the client trusts
 *
 * These are public sample keys from the upstream repository.  They exist so
 * tests/tls/tls_handshake.c can drive a real TLS 1.2 handshake between two
 * NetX Duo IP instances on one Amiga without inventing a PKI.  Never use them
 * for anything that matters.
 *
 * SPDX-License-Identifier: MIT
 */

#ifndef AMINETXDUO_TLS_TEST_CERTS_H
#define AMINETXDUO_TLS_TEST_CERTS_H
...
#endif /* AMINETXDUO_TLS_TEST_CERTS_H */
