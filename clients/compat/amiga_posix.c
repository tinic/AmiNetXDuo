 ...
 *
 *   Every one of those has a two-line AmigaOS answer through dos.library, so
 ...
 *   ftruncate()      SetFileSize() needs a BPTR and we only have a newlib
 *                    descriptor, so this FAILS with EINVAL.  curl calls it in
 *                    exactly one place -- truncating a partially written
 *                    --output file after a failed resume -- and handles the
 *                    failure.  Documented rather than faked.
...
static void stat_fill(struct stat *st, const struct FileInfoBlock *fib)
{
    ...
    /* fib_Protection is ACTIVE LOW for the RWED bits: a set bit means the
       operation is FORBIDDEN.  Getting this backwards would make every file
       look unreadable, so it is spelled out rather than masked in passing. */
...
/*
 * newlib's _fstat() returns success and leaves the caller's struct alone, so
 * a caller that checks S_ISREG() is reading whatever was on the stack.  This
 * one answers the two questions a client actually asks -- "is this a terminal"
 * and "how big is it" -- and says nothing it does not know.
 */
...
/*
 * SetFileSize() takes a BPTR and we are handed a newlib descriptor, with no
 * published way from one to the other.  Failing is the honest answer: curl's
 * single caller (post_per_transfer(), trimming a --output file after a failed
 * resume) checks the return value and reports it.
 */
