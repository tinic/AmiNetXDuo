 ...
 *   This toolchain has no libnet.a at all -- and does not need the wrappers,
 ...
 *   FEATURE TEST does not.  cmake's check_symbol_exists("IoctlSocket", ...)
 *   compiles a five-line program, links it, and reports "not available" when
 *   the link fails on an undefined SocketBase.  That is how curl's build
 *   silently loses HAVE_IOCTLSOCKET_CAMEL_FIONBIO, which is the ONLY way it
 *   knows how to put a socket into non-blocking mode on this platform.
 *
 *   So: a weak definition, in an archive.  A member of an archive is pulled
 *   in only to satisfy an undefined symbol, so during feature detection this
 ...
 *   duplicate.  Weak as well as archived, belt and braces, for the client
 *   that defines it in a translation unit the linker sees later.
 *
