 ...
 *   one linker flag (clients/dropbear/build.sh -S) rather than two source
 *   trees.
 *
 ...
 *   retyped so that a change at the pinned tag is a compile error here and not
 *   a silently mismatched call.
 *
...
/*
 * Ed25519's SHA-512, taken from the libtomcrypt that is already in this
 * binary.  c68k_25519.c has no hash of its own on purpose: this program has
 * one, a TLS build has nx_crypto's, and a second copy would be a second thing
 * to keep right.
 */
...
{
    /*
     * The return value is dropped, and that is Dropbear's interface rather
     * than an oversight: kex-x25519.c's kexcurve25519_comb_key() takes no
     * status from this call.  RFC 7748 section 6.1's all-zero check is not
     * lost -- c68k_x25519() still performs it -- it simply has nowhere to go
     * here, exactly as it has in the TweetNaCl version being replaced.
     */
