...
| 32.10 | Blocking `accept()` could hang forever, and slicing the wait is unsafe: a timeout runs `_nx_tcp_connect_cleanup` and re-sends SYN+ACK on a half-open connection. | `src/bsdsocket/select.c:368` | current, the do-not-slice rule still holds |
