...
| 32.10 | Blocking `accept()` could hang forever, and slicing the wait is unsafe: a timeout runs `_nx_tcp_connect_cleanup` and re-sends SYN+ACK on a half-open connection. | `src/bsdsocket/select.c:368` | superseded in part, measured 2026-08-18. The do-not-slice rule holds for the reason given, but the SYN+ACK it protects against is unreachable since the SYN cache landed: a SYN never reaches the listening socket, so the LISTEN block finds it unbound and sends nothing, and a build with `bsd_accept_once()` reverted to the finite wait emits the same frames (`tests/tcpdrill/scripts/acceptwait.drill`). What the guard costs is accuracy -- see the timeout row in `BACKLOG.md` |
