...
| 5.4 | `-noixemul` is unusable on this newlib toolchain: it breaks `sys/reent.h`. | `cmake/toolchain-m68k-amigaos.cmake:238` | see code |
| 5.4 | The 2026-07 build-spike byte counts. | git history | historical |
| 6.1 | 50 Hz tick. The wakeup source (`UNIT_VBLANK`) is not the time base (`ReadEClock`), and `NX_IP_PERIODIC_RATE` must equal `TX_TIMER_TICKS_PER_SECOND`. | `port/threadx-amiga/inc/tx_port.h:423` | see code |
...
| 9 | Decisions of 2026-07-24. Two of its shipping defaults are inverted in the tree today: TLS and IPv6 both default ON. | `CMakeLists.txt:191`, `CMakeLists.txt:243` | superseded |
| 9 | Target floor. Now any 68000+, OS 2.04+, 1 MB, with pools computed from `AvailMem()`. | `README.md:34`, `include/aminetxduo/netstack.h:272` | superseded |
...
| 13.2 | Never patch vendored source. Use a symbol override, or `-Wl,--wrap`. | `src/tlslib/CMakeLists.txt:62` | current |
...
| 16.9 | A Shell command gets a 4 KB stack and a bsdsocket LVO runs NetX Duo on the caller's stack, so too little stack is an F-line trap and a reboot loop that reads as a timeout. Hence static control blocks, 64 KB `NP_StackSize` minimum, and flush every output line. | `src/tools/httpterm.h:3` | current |
...
| 18 | The hand-written 68020 SHA-256 lost to GCC 15.2 C, 67,656 against 66,687 us, and was deleted. Only the misaligned `MOVE.L` survived. | `src/crypto68k/c68k_sha256.h:59` | see code |
...
| 21 | The size was ours, not newlib. `-ffunction-sections` on two archives plus `--gc-sections` on the commands only, worth -27% to -38%. | `src/tools/CMakeLists.txt:41` | see code |
...
| 25 | This toolchain mis-resolves a 32-bit PC-relative branch from a cross-section tail call twelve bytes short, into the middle of the preceding function. `-fno-optimize-sibling-calls` always travels with `-ffunction-sections`, and a post-link check enforces it. | `cmake/check-pcrel-branches.cmake:177` | see code |
...
| 27 | Ambiguous: two sections are numbered 27. SYN retransmission was flat ~1 s because `NX_TCP_RETRY_SHIFT` defaulted to 0. | `port/netxduo-amiga/inc/nx_user.h:157` | superseded, backoff doubles now |
...
| 33.1 | The keepalive arm is built with a 5 s idle timer so a probe is observable in an emulator run. | `CMakeLists.txt:989` | current, cited as 29 |
...
| 37.5 | Refused `connect()` to a port with a listen request leaked one `AmiSocket` and one packet each, 1009 B/s. | `src/bsdsocket/socket.c:487` | superseded, fixed by `bsd_tcp_abort` |
...
| 44.9 | `DEVS:Networks` is not on the search path a bare device name reaches, so the open retries `Networks/<name>`. | `src/common/compat.c:383` | current |
| 45 | The toolchain has three multilibs keyed on canonical `-mcpu`, so `-m68040` and `-m68030` silently link the 68000 C library. Use `-m68020 -mtune=68040`. | `cmake/toolchain-m68k-amigaos.cmake:245` | see code |
...
| 55 | `AddDomainNameServer()` as ENOSYS made Roadshow's `AddNetInterface` return rc 20 after doing everything right. A server must land in both the NetX Duo DNS client and `ns_Config.resolver`. | `src/bsdsocket/roadshow.c:165` | current |
| 55 | `IP_HDRINCL` is translated, not passed through: TOS, TTL, protocol and destination are mapped onto `nxd_ip_raw_packet_send()` and the caller's header stripped. | `src/bsdsocket/raw.c:635` | current |
| 55 | ARP entries must be emitted as `RTF_LLINFO` routes or Roadshow's `arp` prints nothing. | `src/bsdsocket/routing.c:464` | see code |
| 57 | `-Os` tree-wide, -24% size at no throughput cost, and it must be set after `project()` or `Compiler/GNU` appends `-O3`. | `CMakeLists.txt:43` | superseded in part, eight profiled NetX Duo files build at `-O2` via `AMINETXDUO_HOT_O2` |
...
| 63 | A frame the emulator's host sends to the guest's MAC never returns to that NIC's pcap, so the peer in any bridged test must be a third machine. | `tests/perf/run-fitzbench.sh:118` | see code |
...
| 81 | The memory floor is 1 MB on both builds: fixed cost 432 to 439 KB, and a 1 MB A2000 runs the shipped tools with a 17-packet pool. | `include/aminetxduo/netstack.h:272`, `README.md:34` | current; the two source comments that said 4 MB now say the measured floor |
