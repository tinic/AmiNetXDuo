...
| 90 | An exact 200 Hz profile on a stock physical A1200/EtherLink III found 5.7% true idle. The two necessary payload copies -- card FIFO to `NX_PACKET`, then packet to `recv()` buffer -- total 19.4% of busy time, and no third bulk copy appears. | `docs/PHYSICAL_RX_A1200.md` | current; physical result, not Amiberry |
| 90.1 | The resident `anxnet.device` is 23.6% of physical receive busy time: 12.8% fused PIO copy/checksum and 3.6% `BeginIO`, with every other driver function below 2%. Direct PIO was independently counted and the LTO names were recovered only after an identical final binary was reproduced. | `docs/PHYSICAL_RX_A1200.md` | current |
| 90.2 | On the same physical run event posting plus Exec `Signal` is below 1%, and a maximum-four versus maximum-one receive drain is throughput-neutral at about 1.9 frames per baton transition. Neither can explain the factor-of-two deficit. | `docs/PHYSICAL_RX_A1200.md` | current |
| 90.3 | The strongest unproven physical lead is memory, not a third copy: the affected A1200 has no Fast RAM, while a controlled `FASTMEM=0` arm lost 59%, advertised zero 108 times in ten seconds and left the sender receive-window limited 31.9%. The physical pool/window counters have not yet been captured. | `docs/PHYSICAL_RX_A1200.md`, `tests/perf/run-ackscope.sh` | open inference, not yet a physical A/B |
