...
|---|---|---|---|
| `CreateAmiNetXDuoStatusReport` | **Now — user selected** | High support value: one useful attachment instead of several rounds asking for versions, hardware and status | Print a local, human-readable report and write it to a predictable file by default. Include only allowlisted, passively available facts. No driver load, `OpenDevice`, active probe, network traffic or upload; omit credentials, and make identifying network data opt-in. Mark unavailable facts as such. |
