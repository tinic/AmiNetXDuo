...
| `bsd_lib_open()` blocks inside the library-list `Forbid()` Exec holds across the Open vector | `src/bsdsocket/library.c` | The per-opener setup takes `sb_Lock`, `ami_netdb_load()` reads `DEVS:Internet`, and the first open initializes the loopback-only NetX Duo instance. It does not scan interface definitions or open a network device; explicit `AddNetInterface` does that after the Open vector has returned. `lib_OpenCnt++` holds the library reference across the block |
