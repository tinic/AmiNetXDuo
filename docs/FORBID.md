...
| `bsd_lib_open()` blocks inside the library-list `Forbid()` Exec holds across the Open vector | `src/bsdsocket/library.c` | The per-opener setup takes `sb_Lock` and `ami_netdb_load()` reads `DEVS:Internet`. It no longer initializes NetX Duo, scans interface definitions or opens a network device; explicit `AddNetInterface` does that after the Open vector has returned. `lib_OpenCnt++` holds the library reference across the remaining block |
