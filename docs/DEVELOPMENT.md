...
Where a table would restate a script, this names the script instead: a copy here
goes stale and nothing catches it.
...
the first seven rows below — everything needing neither an emulator nor a
licensed ROM, less `analyze`. Stages can be picked in any combination; the
release set is `host host32 cross web analyze conformance`
...
| `host` | the pre-build gates, then the host test targets and `ctest`; the count is exact against `HOST_TESTS_EXPECTED` in `tools/ci.sh`, so adding a test turns CI red until that line is raised | nothing |
...
The `host` stage also runs the gates that are not ctest — `tools/check-*.sh`,
`tools/test-verdict-selftest.sh` and the per-harness graders
`tests/*/*-verdict-selftest.sh` — each failing the stage on its own, each
existing because the thing it guards runs only where a ROM does, and each
saying in its own header what it checks.

## On-Amiga harnesses

**`tests/HARNESSES` is the index**, kept honest by `tools/check-harnesses.sh`.
Each row is `<path> : <runner> : <note>`; the runner is a file that invokes it,
`chained:<path>`, or `manual` with a reason code.
...
boots to a black screen with an empty log.

**Take no timings above a 68020.** Cycle accounting is off for every model above
...
| `AMI_ERROR`, `AMI_WARN`, `AMI_INFO` | `include/aminetxduo/compat.h` | compiled into every image, shipping ones included; what prints is the runtime `ami_log_level()`, starting at `AMI_LOG_WARN`. A serial capture from a shipped library IS evidence for the error and warning tier and for nothing above it: `SetEnv ANXDLOGLEVEL 2` and restart the network to reach `AMI_INFO`. `AMI_DEBUG` and `AMI_TRACE` are per-packet and still need `AMINETXDUO_DEBUG`. Reaching the serial port at all takes a null modem or Sashimi |
