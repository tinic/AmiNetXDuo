...
| `src/bsdsocket/netmonitor.c` `bsd_mon_list[]`, `bsd_mon_count` | `Forbid()` for short list/refcount changes; callbacks run unlocked | Internal nodes retain in-flight calls and their owning child base; explicit removal and child teardown wait for callbacks already executing |
...
| `src/netstack/netstack_lock.c` singleton lock | `InitSemaphore` under `Forbid()`, both the test and the set inside one | |
| `src/netstack/netstack.c` `ami_ns` | writes under the singleton lock; `bsdsocket.library` publishes its `NX_IP` and packet pool on the master base while the first stack reference is held, then clears both before final shutdown | Library calls resolve NetX objects through their opener base; the base reference is the enforceable lifetime lease |
...
| `TX_THREAD.tx_thread_amiga_exec_wait_nesting` (`port/threadx-amiga/inc/tx_port.h`) | `Forbid()` on every touch; ownership is the ThreadX created-thread registry | A released bracket is owned by the `TX_THREAD` whose lifetime it extends. There is no fixed-capacity table and no `struct Task *` identity to recycle. Foreign teardown calls `ami_netstack_baton_abandon()` before deleting the thread; kernel shutdown resets the diagnostic live count |
...
| `src/tls/ami_tls_crypto.c:296` `ami_rsa_keys[4]`, `:297` `ami_rsa_key_count` | none | Private-key primes matched by modulus bytes rather than by session, holding borrowed pointers into the caller's DER; no slot is reclaimed when a certificate is freed and the fifth loses CRT acceleration permanently. Reached from `TLSOpenA` with `TLSA_Server` and `TLSA_Certificate` (`src/tlslib/tls_server.c:151`), so a server that loads a fifth certificate hits it |
...
| `src/tlslib/tls_netx.c:264` `tls_mutex_slots[]` | allocated and looked up under `Forbid()` (`:290`); the semaphore inside each slot is the lock the callers take | the mutex-to-semaphore map for the vendored TLS code |
