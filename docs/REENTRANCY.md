...
| `src/netstack/netstack.c` `ami_ns` | writes under the singleton lock; raw accessors still rely on the caller's stack reference | `netstack_ip()` and `netstack_pool()` hand out a pointer that shutdown can retire; replace raw reads with an enforceable lease |
