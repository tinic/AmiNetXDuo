...
| 4862 §5.5.3 | An address formed from an autonomous prefix expires with that prefix | fixed 2026-08-04, `nx_icmpv6_process_ra.c`, `nx_ipv6_prefix_list_add_entry.c` | was: the prefix list held the L bit's prefixes only, and the same list is what ages stateless addresses out, so an A=1 L=0 prefix formed an address nothing could remove. The entry now carries the on-link bit rather than standing for it |
