...
| 5961 §3, §4, §3.2 | Challenge ACK required for in-window RST and SYN, **and rate-limited** | `nx_tcp_socket_packet_process.c:234-248`, `:257-271`; RST at any sequence accepted when `RCV.WND==0` at `:165-167`; the ACK at `:226` is unconditional, with no throttle anywhere | One in-window guess resets an established connection. Separately, every out-of-window segment draws an ACK, unbounded — §3.2 requires a limit, and without one a stream of them is an amplifier |
