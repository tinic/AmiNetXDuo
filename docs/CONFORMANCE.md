...
|---|---|---|
| `SBTC_CAN_SHARE_LIBRARY_BASES` | an application **opt-in**, recorded per opener and acted on by nothing, because there is nothing to relax: Roadshow and AmiTCP_NG refuse a caller that is not the opener until this is set (`CHECK_TASK()`), and no call path here looks at the calling task. A SET is accepted; refusing one would deny an application something it already has. What sharing does not buy is the same as theirs — `SIGIO`, `SIGURG` and break signals go to the opener (`select.c:48`, `:747`) and `errno` is per base — plus one timer: `WaitSelect()` with a timeout is served by the first task to ask, and a second is refused `EINVAL` rather than left waiting on a signal bit it does not own | `errno.c:381`, `select.c:456`, `:723` |
...
| Nagle | RFC 1122 §4.2.3.4 says a TCP SHOULD implement it. The default build has none: `TCP_NODELAY` refuses 0 rather than report coalescing that does not happen. An experimental cork exists behind the default-OFF `AMINETXDUO_TCP_CORK` compile-time option; it is not a default Nagle implementation because of its request/response latency cost. The sender silly-window rule is separate: it withholds on the peer's WINDOW, never on write size, which `tests/netstack/host/test_tcp_sws_host.c` asserts with one byte against 512 unacknowledged | `_nx_tcp_socket_sws_send_permitted`, `src/bsdsocket/cork.c` |
