...
| **SACK send side** | Written on fork branch `amiga-tcp-sack-transmit` (`d8af79c5`), never landed, and **measurement says do not land it as it stands**. It merges onto the pinned commit with two additive conflicts, builds clean at +652 bytes, and breaks nothing: 59 cases and 568 checks green. It also changes nothing. Four shapes were put on the wire against it and against its absence, including three duplicate acknowledgements with a later block held, an RTO with a later block held, and nothing acknowledged with only the middle segment held: the frames are identical every time. The reason is that this stack retransmits the OLDEST unacknowledged segment and only that one, repeatedly, until it is acknowledged, so it never reaches a SACKed segment to skip. A skip has nothing to skip until the retransmission loop itself walks the window, which is a larger change than reading the blocks | `nx_tcp_socket_retransmit.c`, `tests/tcpdrill/scripts/sack.drill` s13 |
