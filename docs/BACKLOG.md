...
- **A command that reads the netdb leaks it on the way out.** `ping`,
  `ShowNetStatus` and `AddNetRoute` call `ami_netdb_load()` and never call
  `ami_netdb_free()`. Each links its own copy of `src/config/netdb.c`, and
  `ami_alloc()` is `AllocVec()`, which a Shell command's exit does not reclaim
  -- so every invocation costs the same 12,616 bytes the library's expunge
  used to (fixed there by freeing the tables in `bsd_lib_expunge()`). What
  makes it more than a one-line change is where to put the call: `AddNetRoute`
  alone returns from `main()` in twenty-eight places and none of the three has
  a single exit to hang cleanup on. By inspection, not measured: the drill
  cycles the library, not the commands.
