...
| **TLS 1.3 client** | Does **not** address the row above: 1.3 verifies the same chain against the same root, so the X.509 arithmetic is unchanged. What it buys is one fewer round trip, ChaCha20-Poly1305 in place of a bit-serial GHASH on the bulk path, and reachability as public hosts turn off 1.2. HKDF over the existing HMAC-SHA256 and RSA-PSS are the only new primitives; SHA-384 is avoidable by offering `TLS_CHACHA20_POLY1305_SHA256` and `TLS_AES_128_GCM_SHA256` only | RFC 8446 |
