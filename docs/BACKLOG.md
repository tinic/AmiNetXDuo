...
|---|---|---|
| **The TLS 1.3 EC leg fails on a 68000, and the handshake reports success anyway** | `tests/tls/run-tls13.sh` on `-c 68000`: the RSA-2048 leg passes (60.5 s handshake), the P-256 leg does not. `_nx_secure_tls_session_start()` returns `NX_SUCCESS` and sets `TLSF_HANDSHAKEN` (`tc_Flags` 0x03), `fetch` prints a verified chain and suite 0x1301, and the first `TLSWrite` then returns `NX_NOT_CONNECTED` (0x38) -- so a session is reported up on a transport that is already gone. The peer's view is `UNEXPECTED_EOF_WHILE_READING`: the client vanished mid-handshake. **Ruled out by measurement**: the handshake timeout (probe reads 0xafc8 = 45000 *ticks*, 900 s at `NX_IP_PERIODIC_RATE` 50), IP-thread starvation (`AMI_IP_THREAD_PRIORITY` 2 preempts `AMI_CALLER_PRIORITY` 16), session resumption (cache keyed by host and port), memory (8 MB Fast), and connection order (EC as the *first* connection fails too; it is flaky, not ordered). Connection lifetime before the EOF clusters at **38.5 / 40.7 s at `-k 14`** and **79.9 / 79.4 s at full emulation speed** -- a ratio of about 2, with the *slower* CPU dying *sooner*, which fits neither a fixed timer nor "the client takes too long". 68020 and 68060 pass. Next: a packet capture on the forwarded port (needs sudo on playhouse3, unavailable to the agent) or a peer that records how far the client's flight got, to separate "our stack tore the connection down" from "the server rejected the Finished and closed" | `src/tlslib/tls_conn.c:872`, `tests/tls/run-tls13.sh` |
| **Cold handshake against a three-certificate chain** | `fetch https://` to a current public host fails: the handshake runs 181 s and the peer hangs up, `NX_NOT_CONNECTED` 0x38. Not the trust store, which validates all candidates against `third_party/cacert/cacert.pem`, and not the host: every candidate probed is now three deep. `DEVELOPMENT.md` line 489 already recorded that a three-certificate chain "cannot complete a cold handshake at 13.9 MHz at all"; what changed is that Let's Encrypt rotated to the ISRG `Root YR` hierarchy, so the condition that was specific to Cloudflare is now general. The 6.8 s figure in `tls_resume.c:206` was a two-certificate chain | `tests/tls/run-https.sh` |
