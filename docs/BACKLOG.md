...
| `run-ifdhcp`'s bridged arm always fails because it asserts SLIRP's literals | derive the expected addresses from the arm instead | `tests/tools/run-ifdhcp.sh:167`, `:318` |
...
| The expunge refusal is proven at the library and the port, never at the joint | nothing tests that a failed stop leaves the started flag set | `src/netstack/netstack.c:1852`, `:2192` |
...
| No resolution the lab tests is unaligned | every arm is a multiple of 16 with nothing to pad, which is how 1600 columns shipped | `tests/tools/run-console.sh`, `src/tools/httprtg.c:621` |
...
| The crypto yield hook is installed by `tls.library` alone | anything linking `aminetxduo_tls` directly gets a NULL hook and holds the baton | `src/tlslib/tls_netx.c:130` |
...
| Amiberry cannot eject a PCMCIA card, so removal is unexercised | `pc_on_removed`/`pc_on_inserted` have never fired, nor the reset order | `src/netdev/netdev_pcmcia.c:488`, `:511` |
...
| Two netdev poll bounds are sized in iterations, not in time | 4000 and 20000 expire early on a fast machine, so a slow card is called dead | `src/netdev/el3.c:176`, `:277` |
...
| No arm prices the 64-bit-divisor branch on a 68000 | rtdiv ran on an A1200 only; that branch now enters the 32-iteration fallback once per divide | `src/common/ami_udivdi3.c:185` |
