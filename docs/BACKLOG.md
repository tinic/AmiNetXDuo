...
| Audit 2026-09-20, source cohesion: three shipping monoliths remain (`httpd.c`, `netstack.c`, `netdev_device.c`); the HTTP server still combines protocol dispatch, WebDAV traversal and three browser applications, while the stack and device files each combine lifecycle with policy | continue extracting independently testable policy without changing protocol, lifecycle or hardware behaviour; current counts belong in `wc`, not a stale ledger | `src/tools/httpd.c`, `src/netstack/netstack.c`, `src/netdev/netdev_device.c` |
