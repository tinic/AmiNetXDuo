...
| **A zero-window probe does not arm once the peer has sent us data.** A refused `send()` against a zero window arms the probe on a connection that has only ever sent (`nx_tcp_socket_send_internal.c:1046-1055` captures the caller's first byte at SND.NXT); on one that has received anything it does not, and no probe follows within 6 s. If the peer's window update is then lost the connection stalls for good, which is the one thing the persist timer exists to prevent. Isolated on hardware: client + no inbound data arms (11 and 100 byte payloads both); accepted socket + inbound data does not; **client + inbound data does not** — so it is the inbound data, not the accept path and not the payload size. `tcp.drill` z01 passes only because that case never receives | `nx_tcp_socket_send_internal.c:1046` |
