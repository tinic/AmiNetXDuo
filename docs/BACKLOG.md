...
|---|---|---|
| **A TLS 1.3 session comes up on a transport that is already gone** | `_nx_secure_tls_session_start()` returns `NX_SUCCESS` and sets `TLSF_HANDSHAKEN` (`tc_Flags` 0x03), the client reports a verified chain and suite 0x1301, and the first `TLSWrite` returns `NX_NOT_CONNECTED` (0x38, probed). The peer's view is `UNEXPECTED_EOF_WHILE_READING`: the client vanished mid-handshake. **This is one defect, not the two it was filed as** -- the "68000 EC leg" and the "cold handshake against a three-certificate chain" rows were the same failure, so it is neither 68000-specific nor EC-specific. **Best repro** is now the release e2e: `install/test/run-workbench.sh -a <lha>` on a **68020**, `fetch https://www.iana.org/`, 3 certificates, fails ~19 s in, three of its four checks passing. **Ruled out by measurement**: the handshake timeout (0xafc8 = 45000 *ticks*, 900 s at `NX_IP_PERIODIC_RATE` 50), IP-thread starvation (`AMI_IP_THREAD_PRIORITY` 2 preempts `AMI_CALLER_PRIORITY` 16), the resume cache (keyed by host and port), memory (8 MB Fast), connection order, and CPU speed. Connection lifetime before the EOF clustered at 38.5/40.7 s at `-k 14` against 79.9/79.4 s at full speed -- the *slower* CPU dying *sooner*, which fits neither a fixed timer nor "the client takes too long". **Unexamined**: the window between the handshake returning and the first write, where `TLSOpen` calls `tls_resume_record()` outside the ThreadX bracket to mirror the session to disk. Next: instrument that gap, or capture the forwarded port (needs sudo on playhouse3) to see which end sends the FIN | `src/tlslib/tls_conn.c:681`, `install/test/run-workbench.sh` |
