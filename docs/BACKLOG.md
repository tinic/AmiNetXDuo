...
|---|---|---|
| **Handshake cost against current public chains** | `fetch https://` to any modern host fails: the handshake runs 181.1 s and the peer hangs up, `NX_NOT_CONNECTED` 0x38. Host-independent, `www.iana.org` and `valid-isrgrootx1.letsencrypt.org` alike; the trust store is not implicated, both validate against `third_party/cacert/cacert.pem`. `tls_resume.c:206` records 6.8 s cold for the same handshake, so the cost grew with the chains rather than with us. Which term grew is not isolated | `tests/tls/run-https.sh` |
| **TLS 1.3 client** | Cuts the terms in the row above: one signature verification rather than a chain of RSA exponentiations, and ChaCha20-Poly1305 in place of a bit-serial GHASH. HKDF over the existing HMAC-SHA256 and RSA-PSS are the only new primitives; SHA-384 is avoidable by offering `TLS_CHACHA20_POLY1305_SHA256` and `TLS_AES_128_GCM_SHA256` only | RFC 8446 |
