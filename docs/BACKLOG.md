...
|---|---|---|
| `worst stall 0 ms` is printed beside a non-zero skew | `tx_amiga_tick_worst_stall_ms` is only written inside the clip branch, so a machine that never clips records nothing and the one figure that would explain a skew peak is always 0. The skew counter also has an unsurfaced floor of `TX_AMIGA_VBLANK_DIVIDER` (2) | `port/threadx-amiga/src/tx_initialize_low_level.c:1224-1231`, `:571` |
