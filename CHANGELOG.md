...
- `SO_REUSEPORT` on a UDP socket now shares the port instead of being ignored,
  ...
  datagram still reaches exactly one. The built-in mDNS responder opts its 5353
  socket in, so a second program can now share 5353 with it. `SO_REUSEADDR`
  keeps its TIME-WAIT meaning for TCP, and `SO_REUSEPORT` on a TCP socket still
  sets that same reuse; setting either option on an already-bound UDP socket is
...
- The Installer release gate adds cancellation and four corrupt-download
  ...
  and corrected. Manual card selection without a vendor driver must produce a
  usable `anxnet.device` configuration, bringing the matrix to 38 scenarios.
  Existing-stack fixtures also cover stale `.old`
...
- **`httpd -F` serves a file manager at `/files`.** The WebDAV root and the
  ...
  such as startup-sequence; binary files still download. Text files can be
  edited directly with line numbers, undo, search and keyboard saving. An ETag
  check refuses a save when the file changed after it was opened. Uploads, new
  drawers, renames and deletion use the same DAV namespace. The self-contained
  page follows the browser's light or dark setting, handles Amiga filename
  bytes, and is about 107 KB compressed. An explicit drawer argument remains
  available for a deliberately restricted share
...
can change it. Roadshow starts interfaces from `S/Network-Startup`, AmiTCP_NG
ships the same script, and AmiTCP from `startnet`; none of them opens a network
card from inside the library.
...
for as long as the machine is up, and `sizeof(AmiNetStack)` is the stack's one
runtime allocation. `usergroup.library` and `tls.library` are opened by the
programs that want them.

| | full | minimal | micro |
|---|---|---|---|
| `bsdsocket.library`, file | 338,216 | 213,916 | **168,548** |
| `bsdsocket.library`, in RAM | 321,476 | 202,228 | **159,308** |
| `anxnet.device`, in RAM | 37,308 | 37,308 | 37,312 |
| `sizeof(AmiNetStack)` | 67,652 | 16,792 | **15,528** |
| **RAM while the stack is up** | **426,436** | **256,328** | **212,148** |
...
- Removing one interface no longer takes its sibling off the wire. `S2_ONLINE` and `S2_OFFLINE` are unit commands and every interface on one unit shares one wire, so on a card with two interfaces on `UNIT=0` -- or four -- the first one removed sent the whole unit offline underneath the others. The openers of a unit are counted now, keyed on device, unit and board, and the offline command is issued only by the last interface to leave
- A 3c589 is no longer declared dead on a fast machine. Three waits inside `anxnet.device` -- the command-register wait, the EEPROM read and the coax converter's start-up -- polled a hardware bit a fixed number of register reads rather than for a length of time, so the faster the processor the sooner each expired, and a perfectly good card failed to come up on exactly the machines that should have been the safest. All three are clocked against the raster beam now, with the old count kept as an unconditional floor so no machine waits less than it did before
- An empty expansion slot is no longer reported as a card. Gayle's bus keeps the last byte that was driven on it, so a probe that writes a register and reads the same address back is reading its own write, and both `CheckNetDevice` and the ISA-PnP probe took that echo for a card that was not there. The probe now drives a different byte at a second register between the write and the read, so a floating bus cannot answer correctly
...
- A listening socket no longer halves the receive window of every connection it accepts. The window was divided by the number of sockets the stack had created, and a server has two before a connection exists, so `iperf -s` on an 8 MB A1200 advertised 36 KB where it had 72 KB to offer
- IPv6 receive is four to five times faster: 386 to 1959 kbit/s on an 8 MB A1200, with IPv4 unchanged beside it. The three read queues now share one budget instead of each holding a private number, and the IPv6 one was two frames deep because it had been sized for neighbour discovery rather than for the whole protocol
...
- `NetCapture` writes what is on the wire to a `.pcap` file that Wireshark and tcpdump open, filtered by host, port or protocol. `NetTrace` already did this for a transfer it ran itself; this one captures what other programs are doing
- `ChangeRouteTagList()` changes a route. It was in the vector table and did nothing, so a route could be added and deleted but not altered
- A small write is no longer sent into a window too small to carry it, which is the sender half of RFC 1122's silly-window avoidance. `setsockopt(TCP_NODELAY, 0)` is now refused rather than accepted: there is no Nagle in this stack to turn on, and reporting success said there was
...
- `netstat -s` reports the longest gap between two ticks, and what the tick task cost when it happened. Both numbers were only written when a tick had to be thrown away, so a machine that never threw one away printed `worst stall 0 ms` beside a timer wheel that was demonstrably late
- An X-Surf, X-Surf 100 or X-Surf 500 reaches the internet over IPv6. The driver refuses every multicast address this stack asks it to receive, testing the wrong bit for a group address, so the card never listened for its own solicited-node address and every answer from beyond the local network was dropped one hop away. It now asks for an address in the same hash bucket that the driver will accept
- `fetch`, `nc`, `telnet` and `whois` try every address a name has instead of only the first. A machine whose IPv6 goes nowhere took 191 seconds to fail on a name that had a working IPv4 address all along; it now connects in about 10. `-4` and `-6` still pin the family absolutely
- A transfer that hits a lost packet no longer collapses to a crawl. At the loss a real network shows, two tenths of one percent, one transfer in three fell to as little as 352 KB/s; the slowest of nine is now 1512, for no change in the average rate
...
- A name server a router withdraws stops being used, instead of answering for the rest of the session
