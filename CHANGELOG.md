...
- The archive is built without link-time optimisation, and gives back the size and the speed 0.25.2's notes claimed for it: `bsdsocket.library` is 399,608 bytes where 0.25.2 shipped 352,960, `tls.library` 243,380 where it shipped 185,720, `usergroup.library` 7,684, and `anxnet.device` 36,392. A link-time-optimised build of this stack locked one real A1200 at boot in August, a build of the same tree without it ran, and nothing since has reproduced or explained it -- while every hour this stack has spent on real hardware was bought on a build made this way. So the archive carries the shape the machine has actually booted. The optimisation is not gone and is still what a build from source does by default; what has changed is which of the two a downloaded release contains, and it changes back when that machine has been asked the question properly
