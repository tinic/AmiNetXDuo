...
- `httpd -C` serves this machine's display at `/console`. The browser shows the frontmost screen, so a Preferences editor that opens its own screen appears, and a change of resolution, depth or overscan is carried through. The mouse and keyboard work. A screen where nothing changes costs five bytes a frame; an idle 640x256 four-colour Workbench runs at about 33 frames a second on an A1200 and about four times that on an A3000. There is no password, and anyone who can reach the port has the screen, the keyboard, the mouse and a button that reboots the machine
