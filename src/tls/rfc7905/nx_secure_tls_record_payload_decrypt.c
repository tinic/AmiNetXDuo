...
/* Only its size is ever read: the bound on iv_size before the session IV is
   filled.  It was a file-scope static, which is the shape the block buffer
   above had to be fixed out of, so it does not get to keep it. */
UCHAR                                 save_iv[20];
#endif
UCHAR                                 copy_size;
const NX_CRYPTO_METHOD               *session_cipher_method;
...
UCHAR                                 nonce[13];
        ...
        NX_SECURE_AEAD_CIPHER_CHECK(session_cipher_method -> nx_crypto_algorithm))
    {
        ...
        if (session_cipher_method -> nx_crypto_algorithm == NX_CRYPTO_ENCRYPTION_CHACHA20_POLY1305)
        {
            ...
            /* Exclusive-or the sequence number into the low eight bytes. */
