 ...
 *
 *   It is needed because the modern web has stopped accepting the CBC suites
 *   this client was born with.  `www.google.com` offers ChaCha20-Poly1305 and
 ...
 *   poly1305-donna uses -- and its point is that every partial product fits a
 *   64-bit accumulator with room to add four more, so a block is twenty-five
 *   MULU.L and no carry chain at all.  A 2^32 layout needs twenty multiplies
 ...
 *
 *   Twenty-five MULU.L at 43 cycles is 1,075 cycles a block, 67 a byte, which
 *   is the floor for this construction on a 68020 and about a third of what
 *   AES-128-CBC charges (233 cycles a byte, docs/RESEARCH.md 18.2).
 *
 * CONSTANT TIME: the arithmetic is, because it is add and multiply on data
 * with no table lookup and no branch on a secret.  The final reduction's
 * select is written branch-free for the same reason.  That is a stronger
 * position than c68k_aes.c is in and it costs nothing to keep.
 *
 ...
 * it uint32_t for exactly this class of reason.  ULONG64 is the 64-bit
 * accumulator, the same type the huge-number code calls HN_UBASE2.
 */
 ...
 * is ONE-TIME -- reusing one over two messages hands over r -- which is why
 * the AEAD in c68k_chacha20.h derives a fresh one per record from the cipher
 * itself and this header takes the key rather than generating it.
 */
...
/*
 * Writes the 16-byte tag and leaves the context zeroed, because what is left
 * behind is r and the accumulator and neither should outlive the record.
 */
