 ...
 *   holds the ThreadX scheduler lock and stops the IP thread (netx_call.c).
 *
 *   Loopback does not exist.  nx_ipv6_packet_send() loops a datagram back only
 *   when the destination is ::1 or one of this host's own addresses; a group
 *   is neither.  So IPV6_MULTICAST_LOOP is accepted and reads back 0, and
...
/* fe80::/10.  socket.c has its own copy for the zone notation; two lines is
   cheaper than exporting one. */
 ...
 * up.
 *
 * -1 when the interface has no usable address yet, DAD still running say.
 */
                            ...
                            const struct ipv6_mreq *mreq)
{
    ...
    if (status != NX_SUCCESS)
    {
        /* NX_NO_MORE_ENTRIES is the NX_MAX_MULTICAST_GROUPS cap and
           NX_OVERFLOW is the driver refusing the MAC address; both are
