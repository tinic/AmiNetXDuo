 ...
 *      not recognise this protocol" branch.  A raw socket opened with
 *      IPPROTO_ICMP, the one every ping and traceroute opens, therefore never
 *      sees a byte.  NX_ENABLE_IP_RAW_PACKET_ALL_STACK (port nx_user.h) moves
 *      the hook to the top of the dispatch, ahead of TCP, UDP, ICMP and IGMP.
 *
 *   2. That hook is the filter, and an installed filter turns the queue off
 ...
 * and normal dispatch continues.  A consumed packet saves one copy and breaks
 * the machine: an ICMP echo request claimed by a raw socket is an echo request
 * nobody answers, and an echo reply claimed by one is a reply nx_icmp_ping()
 * never wakes on.  The filter therefore takes a copy for each interested
 * descriptor and always declines.  That is what makes bsdsocktest 132 (ping
 * 127.0.0.1 from a raw socket) work: the reply it waits for is generated by
 * the ICMP layer, which still sees the request.
 *
 ...
 * leaves nx_packet_ip_header at it.  The copy is therefore taken with the
 * pointer wound back, and the original is restored before the decline.
 *
 ...
 *
 * Transmit is the reverse and is not header-included.  The caller writes the
 * protocol payload (an ICMP message, say) and nxd_ip_raw_packet_send()
 ...
 * recursive for its owner, so the filter's own get/put nests harmlessly.  An
 * application task, which reaches here already adopted by ThreadX, is
 * excluded.  Nothing in this file blocks while it holds the mutex.  A receiver
 * that has to wait releases the mutex first and suspends on the socket's own
 * semaphore.
 *
 ...
 *
 * Same budget as the UDP queue in socket.c, because a queued datagram is a
 * whole NX_PACKET out of the one shared pool.  A quarter of the UDP
 * allowance, because a raw socket sees traffic it did not ask for by port,
 * and an application that never reads leaves the queue full.
 */
...
static UINT bsd_raw_filter(NX_IP *ip_ptr, ULONG protocol, NX_PACKET *packet_ptr)
{
    ...
    for (sock = bsd_raw_list; sock != NULL; sock = sock->as_RawNext)
    {
         ...
         * points, because the copy below does not wind back over the IP
         * header.  The test happens here rather than at recv(), which is the
         * point of the option: a blocked type never becomes a packet copy.
         */
...
LONG bsd_raw_open(struct AmiSocketBase *base, AmiSocket *sock)
{
    ...
    if (bsd_raw_installed == 0)
    {
         ...
         * which nothing ever drains, can take a packet.
         */
...
VOID bsd_raw_close(AmiSocket *sock)
{
    ...
    if (ip != NULL)
    {
        ...
        /* The last one out removes the per-packet cost. */
        if (bsd_raw_installed == 0)
        {
    ...
    {
        /*
         * The stack is gone, and the packets went with it. The registry is not
         * the stack's: it is a file-scope static that outlives any NX_IP. A
         * socket left linked hands the next bring-up a list into freed memory,
         * and a bsd_raw_installed that never returns to zero. The filter is
         * then never installed again. Forbid() rather than nx_ip_protection:
         * the mutex lives inside the NX_IP that was just freed.
         */
        Forbid();
 ...
 * comes first because it was supplied for this datagram, where the other two
 * are standing state. The choice happens here and the checksum after it, so a
 * named source is the source the checksum covers.
 */
                         ...
                         ULONG scope, const BsdCmsgSource *src)
{
     ...
     * from a protocol, a TTL and a TOS. A refusal of IP_HDRINCL on that basis
     * cost us traceroute. Roadshow's fails at startup with ENOPROTOOPT
     * (docs/RESEARCH.md 55).
     *
     * A caller that sets IP_HDRINCL usually does not want an arbitrary header.
     * traceroute varies the TTL and the destination per probe, two of the
     * three fields the send call already takes. The header the caller wrote is
     * therefore parsed, its fields are mapped onto the call, and it is
     ...
     * because IP_HDRINCL already took the caller's header off and settled the
     * destination. Transmit fragmentation is available, because src/netstack/
     * calls nx_ip_fragment_enable() for the receive side. It is deliberately
     * not used here: a raw sender that asks for more than the path carries
     * has to be told so, which is what its ping or its traceroute probe is
...
NX_PACKET *bsd_raw_receive(AmiSocket *sock, ULONG wait, UINT *why)
{
    ...
    for (;;)
    {
        ...
        if (packet != NX_NULL)
        {
             ...
             * this, a non-blocking read leaves a credit behind and the next
             * blocking read spins: get() succeeds at once, finds nothing
             * queued, and goes round again immediately.
             */
            if (sock->as_RawSemOk)
...
VOID bsd_raw_source(NX_PACKET *packet, NXD_ADDRESS *addr)
{
     ...
     * than an IP header, so this cannot happen today. That assumption does not
