 ...
 *
 *   Refusing to connect is correct and makes the library useless on the
 *   hardware it was written for: a user with a flat battery gets "certificate
 *   not yet valid" from every site and no way to work out why.  Checking
 *   anyway and letting it fail is the same outcome.
 *
 *   So: skip the validity dates when the clock is obviously unset, check them
 ...
 *   indefinitely against a machine with no clock.  That is a real weakening,
 *   the same one every device with a dead RTC has, and this stack has no
 *   revocation checking of any kind (no OCSP, no CRL fetch), so the stolen-key
 ...
 *   clock was typed in wrong and now reads 2145, which would otherwise reject
 *   every valid certificate as expired and look identical to a real failure.
 *
 *   The floor is a constant rather than __DATE__: a build-date check would
 *   make the binary non-reproducible and make an old build behave differently
