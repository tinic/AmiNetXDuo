...
/*
 * The host-name chain: name_resolution, then DHCP option 12, then an interface
 * file's ID=, then ENV:HOSTNAME. Reported from real hardware: an interface
 * with ID=a1200 was written and the machine kept answering to "a3000", because
 * ID= was parsed and thrown away and a leftover ENV:HOSTNAME was the only
 * source left.
 */
...
static void test_hostname_precedence(void)
{
    ...
    /* The reported machine: ID=a1200 in the interface file, a remnant
       ENV:HOSTNAME of a3000, and nothing else. */
