...
/*
 * The reader runs IP input itself (ami_sana2_rx_input), so its stack has to
 * cover what the IP thread's covers -- AMI_IP_STACK_SIZE, 4096 -- plus the
 * reader's own frames underneath it: the thread body, the drain, the
 * completion and the delivery. There is no MMU, so a frame that does not fit
 ...
 * to and checks ami_sana2_rx_thread against this number. It went from 540
 * bytes to 1664 when IP input moved on to the reader, so 4096 would have left
 * 2432 -- and the script's header says its figures run about 800 light against
 * what tests/stack measures in a guest, because a call through a function
 * pointer is not a symbol in the assembly.
 */
