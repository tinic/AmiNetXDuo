 ...
 * Reference", 3Com 09-0398-002B, August 1994.  NetBSD's elink3reg.h (4-clause
 * BSD) and AROS's etherlink3 (GPL-2.0+) were read but not copied: this tree is
 * MIT.
 *
 * Offset 0x0e is Command when written and Status when read, in every window;
 * the other fourteen bytes are one of eight overlays, chosen by a Command
 * opcode and read back in the top three bits of Status.
 *
 ...
 * argument under it.  EL3_CMD(EL3_C_RESET, 0) is 0x0000, the same word in
 * either byte order, which is why it can be issued before the order is known.
 */
...
/*
 * The low byte of Status is the interrupt cause set, and Acknowledge Interrupt
 * and Set Interrupt Mask take the same bit assignments.  TX_COMPLETE clears
 * only when the transmit status stack is popped, RX_COMPLETE only when the
 * packet is read out and discarded, ADAPTER_FAIL only when the FIFO diagnostic
 * bit is cleared.  INT_LATCH is acknowledged last, after every cause.
 */
 ...
 * any kind, so "group" is every multicast address or none.  Group implies
 * broadcast on this part; broadcast is set anyway rather than relying on it.
 */
...
/*
 * The manufacturer ID is 3Com's EISA code and is hard-wired: every part in the
 * family reads this word and no other.  el3_answers() uses it to decide both
 * that a card is there and which way round the window delivers a word.
 */
 ...
 * ROM size, 11..8 its base, and 4..0 the I/O base.  On PCMCIA the ROM and I/O
 * base fields must all be zero: with a non-zero I/O base the card does not
 * answer I/O, and with it zero the card decodes on the bottom four address
 * lines only, which is why 0x300 works.
 */
 ...
 * reserved and must read back as all ones.  0x3f00 follows from the manual's
 * two statements for a PCMCIA card: the interrupt field must be 3 and the
 * reserved field 0xf.
 */
...
/*
 * The EEPROM is not bit-banged: one word write says read word N, and the answer
 * appears in the data register when the busy bit clears.  A read is quoted at
 * 162 us.
 */
...
/*
 * The operating window.  One PIO port carries every frame in both directions:
 * writes push transmit data, reads pull receive data, and the port does not
 * advance an address -- the chip does.
 */
...
/*
 * Receive status.  Length is only meaningful once INCOMPLETE is clear.  ERROR
 * set means bits 13..11 are the reason; ERROR clear means they are nothing or
 * the dribble-bits note, which is not a reason to drop a frame.
 */
 ...
 * that is not a word.  A 31-deep stack: read the byte, write it back to pop,
 * repeat until the read is zero.  Any error bit disables the transmitter, so
 * TX Enable must follow; jabber and underrun need a TX Reset before that.
 */
...
/*
 * A transmit is two preamble words and then the body.  Word one is the length
 * in bytes in its low eleven bits; word two is ignored and the manual asks that
 * it be written as zero.  The body is padded to a four-byte boundary and the
 * pad is in the FIFO but not in the length.  Frames under the Ethernet minimum
 * are padded by the card and must not be padded here.
 */
 ...
 * global reset runs, so the only option is the manual's millisecond.
 */
