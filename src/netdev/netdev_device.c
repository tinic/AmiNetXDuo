 ...
 *
 * AND IT MUST RUN IN FRONT OF netdev_tx_pump(), NOT AFTER IT.  Behind the pump
 * -- which is where 7acd60c7 put it -- every arriving acknowledgement has its
 * signal held back by the length of the pump, and this rig's TRANSMIT is its
 * ~4.5 ms acknowledgement round trip.  Six alternating rounds measured
 * tcp-tx -1.11%, HEAD's whole range at or below BASE's median, for exactly
 * that.  The receive side of the same arm read +0.36% and could not have read
 * anything: 128 beam units saved of a 1.9 ms frame period is ~1.4%, and iperf
 * at six rounds carries a ~5% within-arm spread.  run-lossgate.sh -d 50 -l 0
 * is the instrument that resolves this size (read_kbs iqr 0.6%).
 */
