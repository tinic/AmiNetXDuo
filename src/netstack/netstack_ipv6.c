/*
 * AmiNetXDuo, the IPv6 half of the stack singleton.  Compiled only in an
 * AMINETXDUO_IPV6 build.  Address modes: LINK-LOCAL, AUTO (RFC 4862
 * stateless autoconfiguration), STATIC and DHCP; DHCPv6 itself, and AUTO's
 * second half, are in netstack_dhcpv6.c.
 *
...
LONG ami_netstack_ipv6_enable(AmiNetStack *ns)
{
    ...
    /*
     * nxd_ipv6_enable() before nxd_icmp_enable(): icmp_enable clears the
     * neighbour and destination caches that ipv6_enable's tables index, and
     * the IPv4-only nx_icmp_enable() never installs the v6 receive path.
     */
    ...
    /* No setter exists, and the fields are read only on the IP thread. */
...
/*
 * The prefix length to report.  NetX Duo stores 10 for a link-local -- the
 * RFC 4291 2.5.6 fe80::/10 allocation -- because _nxd_ipv6_interface_find()
 * compares destinations against that field; the interface has a /64.
 */
 ...
 * Reported from the IP thread rather than waited for: RFC 4862 5.4 puts DAD
 * ahead of an address being used, not ahead of the caller that asked for it.
 */
static VOID ami_ns6_address_changed(NX_IP *ip_ptr, UINT status,
                                    UINT interface_index, UINT address_index,
                                    ULONG *address)
{
    ...
    /*
     * First, and unconditionally: netstack_dhcpv6.c takes NetX Duo's single
     * address-change slot back, so the DHCPv6 client's DAD handler has to be
     * called from here or a duplicate address never produces a DECLINE.
     */
    ...
    case NX_IPV6_ADDRESS_DAD_FAILURE:
        /* RFC 4862 5.4.5: a duplicate link-local stops IPv6 on the interface,
           and NetX Duo has already withdrawn the address. */
    ...
    case NX_IPV6_ADDRESS_STATELESS_AUTO_CONFIG:
        /*
         * nx_icmpv6_process_ra.c leaves the new address TENTATIVE; nothing
         * may use it until the DAD_SUCCESSFUL arm above runs for it.
         */
...
/*
 * Start this interface soliciting a router.  nxd_ipv6_enable() seeds these
 * four fields only for the interfaces that exist when it runs, so one added
 * through AddInterfaceTagList() has a solicit count of zero and never asks.
 */
...
/*
 * A link cycle does not recreate the NX_INTERFACE, so the exhausted
 * solicitation count survives; re-arm an AUTO interface when it is raised.
 */
...
static VOID ami_ns6_configure_interface(AmiNetStack *ns, UWORD i)
{
    ...
    /*
     * RFC 8200 section 5 puts a 1280-octet floor under every link IPv6 runs
     * over, with no fragmentation below it.  IPv4 has no such floor and stays
     * on the interface.
     */
    ...
    {
        /*
         * Already been through here, and everything below it was done then.
         * `index` is not usable after this.
         */
        return;
    }
    ...
    if (cfg->ip6type == AMI_IP6TYPE_AUTO)
    {
         ...
         * field is zero-initialised and zero means enabled.
         */
    ...
    {
        /*
         * LINKLOCAL, STATIC and DHCP need an explicit disable, or a router
         * advertisement adds a global address nobody asked for.  Requires
         * NX_IPV6_STATELESS_AUTOCONFIG_CONTROL in nx_user.h.
         */
    ...
    if (cfg->have_gateway6)
    {
        ...
        /* Already inside the ThreadX bracket; the public wrapper would nest. */
...
/*
 * Put ami_ns6_address_changed() back in NetX Duo's single address-change
 * slot, which nx_dhcpv6_client_create() takes for its own DAD handler.
 * netstack_dhcpv6.c calls this immediately after the create.
 */
                                ...
                                ULONG *origin_out)
{
    ...
    /*
     * The interface's own list, not the flat nx_ipv6_address[] array: that
     * array is shared between interfaces and the loopback ::1 entry.
     */
...
BOOL netstack_ipv6_have_global(VOID)
{
    ...
    for (i = 0; i < (UWORD)NX_MAX_PHYSICAL_INTERFACES && !found; i++)
    {
        for (slot = 0; ; slot++)
        {
            ...
            /* 2000::/3 only: neither a link-local nor fc00::/7 is routed off
...
/*
 * Not zero: the once-a-second tick in nxd_ipv6_prefix_router_timer_tick.c
 * reads zero as already expired.
 */
...
/*
 * _nx_ipv6_packet_send() consults the destination cache before either list,
 * so a route change does not move existing traffic until the entry ages out.
 * Marking the slots invalid is what _nx_invalidate_destination_entry() does.
 */
                              ...
                              const ULONG next_hop[4], UWORD interface_index)
{
    ...
    else
    {
        ...
        /* _nx_ipv6_prefix_list_add_entry() expects nx_ip_protection held. */
        tx_mutex_get(&ns->ns_Ip.nx_ip_protection, TX_WAIT_FOREVER);

        if (ami_ns6_prefix_find(ns, prefix, prefix_len) != NX_NULL)
            status = NX_DUPLICATED_ENTRY;
        else
            /* A route added by hand is on-link by definition. */
                               ...
                               ULONG addr_out[4])
{
    ...
    if (interface_index >= 0)
        nxif = &ns->ns_Ip.nx_ip_interface[interface_index];

    /*
     * The same RFC 6724 routine the send path uses (src/ipv6/ipv6_srcsel.c),
     * so getsockname() reports the address the packets carry.  The address
     * and interface tables are owned by nx_ip_protection.
     */
    scratch[0] = dest[0];
    scratch[1] = dest[1];
    scratch[2] = dest[2];
