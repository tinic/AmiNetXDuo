 ...
 *
 * Nothing here knows about DNS. It drives an opaque "ask once, waiting this
 * long" function, which keeps it testable on the development host.
 *
...
/* What one attempt made of the wait it was given. */
typedef enum
{
    AMI_NET_ASK_ANSWERED = 0,   /* got what the caller wanted; stop         */
    AMI_NET_ASK_SILENT,         /* nobody answered; ask again               */
    AMI_NET_ASK_REFUSED         /* answered, but not with an address; stop  */
} AmiNetAskResult;
 ...
 *
 * `budget` is the whole lookup in ticks. Each attempt is charged what it
 * actually took rather than what it was allowed, because one attempt walks
 * every configured server and so can cost several times its own wait.
 *
 * An attempt that comes back before its wait expired means every server it
 * asked answered something, and a server that answers is not going to answer
 * differently in a second; the ladder stops there rather than spending the rest
 * of the budget re-asking. That is also what keeps a mistyped host name fast
 * while a black-holed one still gets the full budget.
 */
