...
static VOID show_counters(const char *name, const AmiSana2Stats *st)
{
    ...
    /* The card's own transmit trouble, from anxnet.device's special
       statistics.  A frame the wire never saw shows up here and nowhere
       else: the shim counted its write as sent. */
                           ...
                           BOOL readable, BOOL described)
{
    ...
    if (live != NULL && live->nx_device[0] != '\0')
    {
        ...
        tool_printf("\nInterface %s (%s unit %ld)\n",
                    (LONG)cfg->name,
                    (LONG)(same && cfg->device[0] != '\0' ? cfg->device
                                                          : live->nx_device),
                    (LONG)live->nx_unit);
