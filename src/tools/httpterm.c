    ...
    while (done < len)
    {
        ...
        switch (b)
        {
            ...
            case (UBYTE)'n':                /* DSR                          */
                if (term_seq_has(6UL))
                {
                    /* CSI 6 n, "where is the cursor".  Swallowed, not passed
                       on: the browser would answer it, and its answer crosses
                       the LAN to arrive in the input after the program that
                       asked has gone on to read a key -- which is then the
                       answer's bytes, not the keystroke.  More asks it after
                       every line and under its pager prompt, so forwarded it
                       turned a page-down into a screen of "--- More ---".
                       More counts lines on its own when no answer comes. */
                    term_seq_n   = 0;
                    term_seq_esc = 0;
                    break;
                }
