 ...
 * and a lock nobody asks about costs nothing to leave lying.  Every lookup
 * below runs it first, so a caller does not have to.
 */
...
/* Does a request carrying these two If: tokens hold `l`?  No lock is everybody's. */
...
/*
 * `path` and everything under it: a Depth: infinity method may not step over a
 * name locked underneath it.
 */
...
/* Empties the table.  For tests; httpd starts with it zeroed. */
